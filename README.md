...
### Chain Switching
The app maintains a map of EVM chain params and calls `wallet_addEthereumChain` automatically if the target chain isn't in the user's wallet yet:
...
- The floating animation uses CSS `@keyframes float` (gentle up/down + tilt)
- The timestamp updates every second showing live UTC time
...
Each minted NFT is randomly assigned one of these 20 cryptocurrencies:
